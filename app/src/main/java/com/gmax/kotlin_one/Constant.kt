package com.gmax.kotlin_one

class Constant {

    companion object{
        val APP_SECRET : String = "99fcf7399865105573df904f72888f19"

        val BASE_OUR_URL_1 : String = "http://111.231.54.111/"
        val BASE_OUR_URL_2 : String = "http://118.178.227.194/"

        val MAIN_EXIT:String = "exit"
        val OTHER_NOT_EXIT:String = "not_exit"
        val CLICK_NUM:String = "click_num"

        val CHANGE_USER = "change_user"

        val USER_TOKEN : String = "user_token"
        val USER_OBJ_ID : String = "user_obj_id"
        val USER_PWD : String = "user_pwd"
        val USER_INFO : String = "user_info"
        val REMEBER_CB : String = "remeber_cb"
        val AUTO_CB : String = "auto_cb"

        val USER_PHONE : String = "user_phone"
        val NICK_NAME : String = "nick_name"
        val USER_HEADPIC : String = "user_headpic"
        val USER_DOVECOTE : String = "user_dovecote"
        val USER_CODE : String = "user_code"
        val USER_AGE : String = "user_age"
        val USER_BIRTH : String = "user_birth"
        val USER_SEX : String = "user_sex"
        val USER_YEAR : String = "user_year"
    }
}